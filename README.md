...
DebtGuard is being built progressively through three major stages:

- Replicate 1 -> Detection
- Replicate 2 -> Simulation
- Replicate 3 -> Optimization

The staged approach exists because the project solves a layered problem. Reliable optimization is not credible without a trustworthy simulation layer, and simulation is not useful without a solid detection layer. Building the system progressively allows each layer to establish a clear product value and a clear technical foundation before the next level of intelligence is introduced.
...
Replicate 3 extends the simulator into a decision engine that can search for better strategies rather than requiring the user to invent every scenario manually.
...
Replicate 3 focuses on the long-term question:

**What is the best possible decision?**

This stage extends the project beyond manual scenario testing. Instead of requiring the user to try one scenario at a time, the system begins to explore a larger decision space programmatically.

At this stage, the platform may:

- explore many possible strategies
- simulate outcomes
- compare paths
- identify promising or optimal actions

This stage may draw on methods such as:

- policy search
- heuristic optimization
- reinforcement-learning-inspired concepts
- strategy ranking systems

The purpose of Replicate 3 is not to create false certainty or pretend that there is always a single objectively correct answer. Its purpose is to help users and advisors search the space of reasonable actions more effectively. In many cases, several strategies may be acceptable, but some will dominate others on relevant dimensions such as payoff speed, resilience, or liquidity preservation.

Replicate 3 transforms DebtGuard from a simulator into an intelligent decision-support engine. Instead of merely answering "what happens if I do this?", the system starts answering "which classes of actions appear most promising given my current state and constraints?"

This is the long-term vision of the project and the stage where the system becomes most intellectually interesting. It moves from explanation of trajectories to structured search over decisions.
...
Fourth, the architecture should remain modular. Detection, simulation, explanation, storage, and optimization should be separable components so the system can evolve without collapsing into an opaque monolith.
...
Near-term and long-term development can be framed along the same staged progression:
...
- introduce Optimization through search, ranking, and constraint-aware recommendation
